...
#include <AMReX_HypreABecCecLap3.H>
#endif
...
MLEBABecCecLap::MLEBABecCecLap (const Vector<Geometry>& a_geom,
...
MLEBABecCecLap::makeFactory (int amrlev, int mglev) const
{
...
void
MLEBABecCecLap::define (const Vector<Geometry>& a_geom,
                     const Vector<BoxArray>& a_grids,
                     const Vector<DistributionMapping>& a_dmap,
                     const LPInfo& a_info,
                     const Vector<EBFArrayBoxFactory const*>& a_factory)
{
    BL_PROFILE("MLEBABecCecLap::define()");
    ...
    MLCellABecCecLap::define(a_geom, a_grids, a_dmap, a_info, _factory);
        ...
        for (int mglev = 0; mglev < m_num_mg_levels[amrlev]; ++mglev)
        {
            ...
            for (int idim = 0; idim < AMREX_SPACEDIM; ++idim)
            {
                ...
                m_c_coeffs[amrlev][mglev][idim].define(ba,
                                                       m_dmap[amrlev][mglev],
                                                       ncomp, ng, MFInfo(), *m_factory[amrlev][mglev]);
                m_c_coeffs[amrlev][mglev][idim].setVal(0.0);
            }
...
MLEBABecCecLap::~MLEBABecCecLap ()
{}

void
MLEBABecCecLap::setPhiOnCentroid ()
{
    m_phi_loc = Location::CellCentroid;
}

void
MLEBABecCecLap::setScalars (Real a, Real b, Real c)
{
    m_a_scalar = a;
    m_b_scalar = b;
    m_c_scalar = c;
...
void
MLEBABecCecLap::setRelaxation (Real omega) noexcept
{
   m_omega = omega;
}
...
void
MLEBABecCecLap::setACoeffs (int amrlev, Real alpha)
{
...
void
MLEBABecCecLap::setBCoeffs (int amrlev, const Array<MultiFab const*,AMREX_SPACEDIM>& beta,
                         Location a_beta_loc)
{
    const int ncomp = getNComp();
...
void
MLEBABecCecLap::setBCoeffs (int amrlev, Real beta)
{
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
...
void
MLEBABecCecLap::setBCoeffs (int amrlev, Vector<Real> const& beta)
{
    const int ncomp = getNComp();
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
        for (int icomp = 0; icomp < ncomp; ++icomp) {
...
void
MLEBABecCecLap::setCCoeffs (int amrlev, const Array<MultiFab const*,AMREX_SPACEDIM>& eta,
                         Location a_eta_loc)
{
    const int ncomp = getNComp();
    const int eta_ncomp = eta[0]->nComp();

    m_eta_loc     = a_eta_loc;

    AMREX_ALWAYS_ASSERT(eta_ncomp == 1 || eta_ncomp == ncomp);
...
void
MLEBABecCecLap::setCCoeffs (int amrlev, Real eta)
{
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
        m_c_coeffs[amrlev][0][idim].setVal(eta);
    }
    m_needs_update = true;
    m_eta_loc     = Location::FaceCenter;
}

void
MLEBABecCecLap::setCCoeffs (int amrlev, Vector<Real> const& eta)
{
    const int ncomp = getNComp();
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
        for (int icomp = 0; icomp < ncomp; ++icomp) {
            m_c_coeffs[amrlev][0][idim].setVal(eta[icomp]);
        }
    }
    m_needs_update = true;
    m_eta_loc     = Location::FaceCenter;
...
void
MLEBABecCecLap::setEBDirichlet (int amrlev, const MultiFab& phi, const MultiFab& beta)
{
    const int ncomp = getNComp();
...
void
MLEBABecCecLap::setEBDirichlet (int amrlev, const MultiFab& phi, Real beta)
{
    const int ncomp = getNComp();
    bool phi_on_centroid = (m_phi_loc == Location::CellCentroid);
    if (m_eb_phi[amrlev] == nullptr) {
...
void
MLEBABecCecLap::setEBDirichlet (int amrlev, const MultiFab& phi, Vector<Real> const& hv_beta)
{
    const int ncomp = getNComp();
    bool phi_on_centroid = (m_phi_loc == Location::CellCentroid);
    if (m_eb_phi[amrlev] == nullptr) {
...
void
MLEBABecCecLap::setEBHomogDirichlet (int amrlev, const MultiFab& beta)
{
    const int ncomp = getNComp();
    const int beta_ncomp = beta.nComp();
    bool phi_on_centroid = (m_phi_loc == Location::CellCentroid);
...
void
MLEBABecCecLap::setEBHomogDirichlet (int amrlev, Real beta)
{
    const int ncomp = getNComp();
    bool phi_on_centroid = (m_phi_loc == Location::CellCentroid);
    if (m_eb_phi[amrlev] == nullptr) {
...
void
MLEBABecCecLap::setEBHomogDirichlet (int amrlev, Vector<Real> const& hv_beta)
{
    const int ncomp = getNComp();
    bool phi_on_centroid = (m_phi_loc == Location::CellCentroid);
    if (m_eb_phi[amrlev] == nullptr) {
...
void
MLEBABecCecLap::averageDownCoeffs ()
{
    ...
    for (int amrlev = 0; amrlev < m_num_amr_levels; ++amrlev) {
        for (int mglev = 0; mglev < m_num_mg_levels[amrlev]; ++mglev) {
            for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
                ...
                m_c_coeffs[amrlev][mglev][idim].FillBoundary(m_geom[amrlev][mglev].periodicity());
            }
        }
    }
}
...
void
MLEBABecCecLap::averageDownCoeffsSameAmrLevel (int amrlev, Vector<MultiFab>& a,
                                            ...
                                            const Vector<MultiFab*>& b_eb)
{
    ...
    for (int mglev = 1; mglev < nmglevs; ++mglev)
    {
        ...
        amrex::EB_average_down_faces(amrex::GetArrOfConstPtrs(c[mglev-1]),
                                     amrex::GetArrOfPtrs(c[mglev]),
                                     ratio, 0);
...
void
MLEBABecCecLap::averageDownCoeffsToCoarseAmrLevel (int flev)
{
    auto const& fine_a_coeffs = m_a_coeffs[flev  ].back();
    auto const& fine_b_coeffs = m_b_coeffs[flev  ].back();
    auto const& fine_c_coeffs = m_c_coeffs[flev  ].back();
    ...
    amrex::EB_average_down_faces(amrex::GetArrOfConstPtrs(fine_c_coeffs),
...
void
MLEBABecCecLap::prepareForSolve ()
{
...
void
MLEBABecCecLap::compGrad (int amrlev, const Array<MultiFab*,AMREX_SPACEDIM>& grad,
                       MultiFab& sol, Location grad_loc) const
{
    BL_PROFILE("MLEBABecCecLap::compGrad()");
    ...
    for (MFIter mfi(sol, mfi_info); mfi.isValid(); ++mfi)
    {
        ...
        if (fabtyp == FabType::covered) {
            ...
            AMREX_LAUNCH_HOST_DEVICE_LAMBDA_DIM (
                fbx, txbx,
                {
                    mlebabecceclap_grad_x(txbx, gx, s, apx, fcx, msk, dxi, ncomp, phi_on_centroid);
                }
                , fby, tybx,
                {
                    mlebabecceclap_grad_y(tybx, gy, s, apy, fcy, msk, dyi, ncomp, phi_on_centroid);
                }
                , fbz, tzbx,
                {
                    mlebabecceclap_grad_z(tzbx, gz, s, apz, fcz, msk, dzi, ncomp, phi_on_centroid);
                }
            );
        } else {
            ...
            AMREX_LAUNCH_HOST_DEVICE_LAMBDA_DIM (
                fbx, txbx,
                {
                    mlebabecceclap_grad_x_0(txbx, gx, s, ax, dxi, ncomp);
                }
                , fby, tybx,
                {
                    mlebabecceclap_grad_y_0(tybx, gy, s, ay, dyi, ncomp);
                }
                , fbz, tzbx,
                {
                    mlebabecceclap_grad_z_0(tzbx, gz, s, az, dzi, ncomp);
                }
            );
        }
    }
...
void
MLEBABecCecLap::normalize (int amrlev, int mglev, MultiFab& mf) const
{
    ...
    AMREX_D_TERM(Real dhxc = m_c_scalar*dxinvarray[0];,
                 Real dhyc = m_c_scalar*dxinvarray[1];,
                 Real dhzc = m_c_scalar*dxinvarray[2];);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(mf, mfi_info); mfi.isValid(); ++mfi)
    {
        ...
        else if (fabtyp == FabType::singlevalued)
        {
            ...
            AMREX_LAUNCH_HOST_DEVICE_LAMBDA ( bx, tbx,
            {
                mlebabecceclap_normalize(tbx, fab, ascalar, afab,
...
void
MLEBABecCecLap::restriction (int amrlev, int cmglev, MultiFab& crse, MultiFab& fine) const
{
...
void
MLEBABecCecLap::interpolation (int amrlev, int fmglev, MultiFab& fine, const MultiFab& crse) const
{
    BL_PROFILE("MLEBABecCecLap::interpolation()");
...
void
MLEBABecCecLap::averageDownSolutionRHS (int camrlev, MultiFab& crse_sol, MultiFab& crse_rhs,
...
void
MLEBABecCecLap::applyBC (int amrlev, int mglev, MultiFab& in, BCMode bc_mode, StateMode s_mode,
                      const MLMGBndry* bndry, bool skip_fillboundary) const
{
    BL_PROFILE("MLEBABecCecLap::applyBC()");
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(imaxorder <= 4, "MLEBABecCecLap::applyBC: maxorder too high");
                ...
                for (int icomp = 0; icomp < ncomp; ++icomp) {
                    ...
                    else // irregular
                    {
                        ...
                        if (idim == 0) {
                            AMREX_LAUNCH_HOST_DEVICE_LAMBDA (
                            blo, tboxlo, {
                            mlebabecceclap_apply_bc_x(0, tboxlo, blen, iofab, mask, ap,
                                                   bctlo, bcllo, bvlo,
                                                   imaxorder, dxi, flagbc, icomp);
                            },
                            bhi, tboxhi, {
                            mlebabecceclap_apply_bc_x(1, tboxhi, blen, iofab, mask, ap,
                                                   bcthi, bclhi, bvhi,
                                                   imaxorder, dxi, flagbc, icomp);
                            });
                        } else if (idim == 1) {
                            AMREX_LAUNCH_HOST_DEVICE_LAMBDA (
                            blo, tboxlo, {
                            mlebabecceclap_apply_bc_y(0, tboxlo, blen, iofab, mask, ap,
                                                   bctlo, bcllo, bvlo,
                                                   imaxorder, dyi, flagbc, icomp);
                            },
                            bhi, tboxhi, {
                            mlebabecceclap_apply_bc_y(1, tboxhi, blen, iofab, mask, ap,
                                                   bcthi, bclhi, bvhi,
                                                   imaxorder, dyi, flagbc, icomp);
                            });
                        } else {
                            AMREX_LAUNCH_HOST_DEVICE_LAMBDA (
                            blo, tboxlo, {
                            mlebabecceclap_apply_bc_z(0, tboxlo, blen, iofab, mask, ap,
                                                   bctlo, bcllo, bvlo,
                                                   imaxorder, dzi, flagbc, icomp);
                            },
                            bhi, tboxhi, {
                            mlebabecceclap_apply_bc_z(1, tboxhi, blen, iofab, mask, ap,
                                                   bcthi, bclhi, bvhi,
                                                   imaxorder, dzi, flagbc, icomp);
                            });
                        }
...
void
MLEBABecCecLap::apply (int amrlev, int mglev, MultiFab& out, MultiFab& in, BCMode bc_mode,
                    StateMode s_mode, const MLMGBndry* bndry) const
{
    BL_PROFILE("MLEBABecCecLap::apply()");
    applyBC(amrlev, mglev, in, bc_mode, s_mode, bndry);
    Fapply(amrlev, mglev, out, in);
}

void
MLEBABecCecLap::update ()
{
...
void
MLEBABecCecLap::getEBFluxes (const Vector<MultiFab*>& a_flux, const Vector<MultiFab*>& a_sol) const
{
    BL_PROFILE("MLEBABecCecLap::getEBFluxes()");
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
            ...
            for (MFIter mfi(*a_flux[amrlev], mfi_info); mfi.isValid(); ++mfi)
            {
                ...
                if (fabtyp == FabType::covered || fabtyp == FabType::regular) {
                    ...
                    {
                        mlebabecceclap_ebflux(i,j,k,n,febfab, xfab, flagfab, vfracfab,
                                           AMREX_D_DECL(apxfab,apyfab,apzfab),
...
MLEBABecCecLap::makeHypre (Hypre::Interface hypre_interface) const
{
    auto hypre_solver = MLCellABecCecLap::makeHypre(hypre_interface);
    auto ijmatrix_solver = dynamic_cast<HypreABecCecLap3*>(hypre_solver.get());
...
#ifdef AMREX_USE_PETSC
std::unique_ptr<PETScABecCecLap>
MLEBABecCecLap::makePETSc () const
{
    auto petsc_solver = MLCellABecCecLap::makePETSc();
